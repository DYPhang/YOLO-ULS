...
__all__ = [ 'Detect_Efficient', 'MyDetect']
...
class MyDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        #self.stem = nn.ModuleList(nn.Sequential(LMGConv(x), Conv_GN(x, x, 1)) for x in ch)
        #self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 1),LMGConv(x)) for x in ch)
        self.stem = nn.ModuleList(nn.Sequential(Conv_GN(x, x, 1),LMGConv(x)) for x in ch)
...
class Detect_Efficient(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.stem = nn.ModuleList(nn.Sequential(LMGConv(x), Conv(x, x, 1)) for x in ch) # one EMSConvP, one 1x1 Conv
        self.cv2 = nn.ModuleList(nn.Conv2d(x, 4 * self.reg_max, 1) for x in ch)
        self.cv3 = nn.ModuleList(nn.Conv2d(x, self.nc, 1) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
