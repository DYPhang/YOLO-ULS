...
from ultralytics.nn.u_modules import *
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect,Detect_Efficient, Segment,MyDetect)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect,Detect_Efficient,Segment,  Pose,MyDetect)):
            s = 640  # 2x min stride
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect,Detect_Efficient,Segment,MyDetect):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 C2f_LMGConv,C2f_LMGConv_GCCA,nn.Conv2d,ADown, V7DownSampling):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_LMGConv, C2f_LMGConv_GCCA):
        ...
        elif m in (Detect, Detect_Efficient, Segment, Pose, MyDetect):
        ...
        elif m in {EMA, SpatialAttention,  CoordAtt, CBAM,  LMGConv, GCCA, MLCA,CBAM,EMA,ChannelAttention,SpatialAttention,
                   CoordAtt,DySample}:
            c2 = ch[f]
            args = [c2, *args]
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect,Detect_Efficient,MyDetect)):
                return 'detect'
